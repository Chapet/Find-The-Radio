...
public class ScavengingController : MonoBehaviour
{
    ...
    */
    ...
    void Scavenge()
    {
        ...
        {
            popUpResult.PopResult(itemsFound, (health: (old: oldHealth, now: player.currentHealth),
                    hunger: (old: oldHunger, now: player.currentHunger),
                    thirst: (old: oldThirst, now: player.currentThirst),
                    ernery: (old: oldEnergy, now: player.currentEnergy)),
                scavengeLog.ToArray());
        }
